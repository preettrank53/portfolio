    ...
    >
      ...
      <div className="md:col-span-1">
        <div className="flex flex-col gap-1 mt-1">
          <span className="text-xs text-[var(--theme-muted)] font-mono tracking-wider block">
            {post.date}
          </span>
          {post.duration && (
      ...
      <div className="md:col-span-3 flex flex-col min-w-0">
        ...
        <div className="flex flex-row gap-4 items-start mb-2">
          {/* Company Logo box */}
          <div className="w-12 h-12 md:w-14 md:h-14 border border-[var(--theme-border)] rounded-sm overflow-hidden bg-white flex-shrink-0 relative">
            {post.logoUrl ? (
              <ImageWithFallback 
                src={post.logoUrl} 
              ...
              <div className="w-full h-full bg-zinc-900 flex items-center justify-center font-bold text-[var(--theme-text)]0 text-lg font-sans">
          ...
          <div className="flex-1 flex flex-col min-w-0">
            <h3 className="text-xl md:text-2xl font-bold tracking-tight text-[var(--theme-text)] leading-snug">
              {post.title}
            </h3>
            <div className="text-sm text-[var(--theme-muted)] mt-0.5">
              {post.company}{post.type ? ` · ${post.type}` : ""}
            </div>
            {post.location && (
              <div className="text-xs font-mono text-[var(--theme-text)]0 mt-0.5">
                {post.location}
              </div>
            )}
          </div>
        ...
        {post.description && Array.isArray(post.description) && (
          <ul className="list-disc list-outside pl-4 space-y-2 mt-4 mb-4">
            {post.description.map((bullet, idx) => (
              <li key={idx} className="text-sm text-[var(--theme-muted)] leading-relaxed font-sans font-medium">
                {bullet}
              </li>
            ))}
          </ul>
        ...
        <div className="flex items-center justify-end pt-2 text-[var(--theme-muted)]">
